...
class Games {
	...
	leaderboard: Array<{ gameType: string; playerName: string; score: number }>;

	/**
	 * Constructor for Games class
	 * @param playerName - Player name to be used in game
	 *
	 ...
	 */
	constructor(playerName: string = "Random Player") {
...
class RPS extends Games {
	...
	async rps(playerChoice: number): Promise<void> {
		const ENEMY_CHOICE_DISPLAY = document.getElementById("enemyChoiceDisplay") as HTMLElement;
		const PLAYER_CHOICE_DISPLAY = document.getElementById(
			"playerChoiceDisplay",
		) as HTMLElement;
		const PLAYER_CONTAINER = document.getElementById("playerContainer") as HTMLElement;
		const ENEMY_CONTAINER = document.getElementById("enemyContainer") as HTMLElement;
		const WIN_TEXT = document.getElementById("determineWin") as HTMLElement;
	...
	upgradeAtk() {
		...
		if (
			...
			if (this.clickHero.currentAtkUp >= MAX_ATK) {
				...
				ATK_PRICE_DISPLAY.innerHTML = String(this.clickHero.upPrice);
			}
		}
	}
...
class Pokemon extends Games {
	private pokemon: {
		maxStage: number;
		currentStage: number;
		currentData: { type: string; name: string; image: string } | null;
		isLoading: boolean;
		trials: number;
	};
	 ...
	 */
	async guessPokemonType(): Promise<
		{ type: string; name: string; image: string } | undefined
	> {
		try {
			...
			const RANDOM_TYPE =
				POKEMON_TYPES[Math.floor(Math.random() * POKEMON_TYPES.length)];
	 ...
	 */
	renderPokemonGame(data: { type: string; name: string; image: string }) {
		const POKEMON_IMG = document.getElementById(
			"pokemonImage",
		) as HTMLImageElement;
...
// Init Games
const LOCAL_NAME: string | null = localStorage.getItem("playerName");
const GAMES = new Games(LOCAL_NAME || undefined);

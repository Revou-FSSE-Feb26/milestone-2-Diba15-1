...
class RPS extends Games {
	...
	constructor() {
		...
		this.ENEMY_CHOICE_DISPLAY = document.getElementById(
			"enemyChoiceDisplay",
		) as HTMLElement;
		this.PLAYER_CHOICE_DISPLAY = document.getElementById(
			"playerChoiceDisplay",
		) as HTMLElement;
		this.PLAYER_CONTAINER = document.getElementById(
			"playerContainer",
		) as HTMLElement;
		this.ENEMY_CONTAINER = document.getElementById(
			"enemyContainer",
		) as HTMLElement;
...
class ClickHero extends Games {
	...
	constructor() {
		...
		this.ATK_PRICE_DISPLAY = document.getElementById(
			"atk-price",
		) as HTMLElement;
...
class Pokemon extends Games {
	...
	constructor() {
		...
		this.POKEMON_IMG = document.getElementById(
			"pokemonImage",
		) as HTMLImageElement;
		...
		this.CHOICE_CONTAINER = document.getElementById(
	...
	async guessPokemonType(): Promise<PokemonDetail | undefined> {
		try {
			...
			const TYPE_RESPONSE: Response = await fetch(
				`${BASE_API}type/${RANDOM_TYPE}`,
			);
	...
	checkAnswer(userGuess: string): void {
		...
		if (userGuess === this.pokemon.currentData.type) {
			...
			this.POKEMON_NAME.textContent =
				this.pokemon.currentData.name.toUpperCase();
